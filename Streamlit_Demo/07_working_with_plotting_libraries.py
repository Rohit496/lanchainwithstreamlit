...
corr_plot = sns.heatmap(
    df[["mpg", "cylinders", "displacement", "horsepower"]].corr(), annot=True
)
st.pyplot(fig)
...
# Map
st.subheader("Map")
temp = pd.read_csv("data/weather_data.csv")
st.write(temp)
...
alt_chart = (
    alt.Chart(df)
    .mark_circle()
    .encode(x="mpg", y="horsepower", color="origin", tooltip=["name", "year"])
    .interactive()
)
st.write(alt_chart)
